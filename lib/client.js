...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function LockScreen() {
			...
			// Solid color (auto-contrast) or a full-bleed background image — no
			// translucent mask, no blur, no card box.
			...
			var faintSub = "rgba(28, 30, 33, .45)";
			if (fg === "#f2f4f7") {
				faintBorder = "rgba(255, 255, 255, .28)";
				faintSub = "rgba(242, 244, 247, .5)";
			}
			var inputStyle = {
				width: "100%",
				boxSizing: "border-box",
			...
			return h("div", { style: layer },
				h("form", { onSubmit: submit, style: { width: "min(92vw, 320px)" } },
					h("div", { style: { fontSize: 15, fontWeight: 600, color: fg, textAlign: "center", marginBottom: 4 } }, t("unlock.title")),
					h("div", { style: { fontSize: 12, color: faintSub, textAlign: "center", marginBottom: 18 } }, t("unlock.sub")),
		...
		function apply(ctx) {
			...
			// CSS rule flattens them while the sidebar is expanded. It degrades
			// gracefully (back to stacked) if the class names change in a newer build.
			ctx.effect(function () {
				var el = document.createElement("style");
				el.setAttribute("data-dsh-simple-auth", "sidebar-footer");
				el.textContent = [
					".hHd-Xa_root:not(.hHd-Xa_collapsed) .hHd-Xa_footArea{flex-direction:row-reverse;align-items:center;gap:8px;}",
					".hHd-Xa_root:not(.hHd-Xa_collapsed) .hHd-Xa_settingsArea{flex:1;width:auto;min-width:0;}",
					".hHd-Xa_root:not(.hHd-Xa_collapsed) .hHd-Xa_footerActions{width:auto;flex:none;}",
				].join("");
