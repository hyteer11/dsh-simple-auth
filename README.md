...
| # | 需求 | 实现 |
| --- | --- | --- |
| 1 | 可配置免认证有效期 | `sessionTtl`（默认 7 天）；登录页勾选「记住我」用更长的 `rememberTtl`（默认 30 天）。有效期内无需再认证；**手动退出**（设置 → 退出登录）立即失效；**主动锁定**（Ctrl+L）后必须重新输入密码 |
| 2 | Ctrl+L 快捷锁定界面 | 客户端快捷键 Ctrl/Cmd+L 弹出全屏锁定遮罩，解锁需验证密码；切换标签页（页面隐藏）也会自动锁定（隐私保护） |
...
- 操作：**锁定界面（Ctrl+L）**、**退出登录**；
- **锁定界面外观**：背景可设为 **默认遮罩** / **纯色** / **背景图**（纯色模式按所选颜色的亮度自动切换文字深浅，视觉自然；背景图 URL 输入即生效）。偏好保存在浏览器本地；
- **修改密码**：原密码 + 新密码 ×2；
- **禁用认证**：需当前密码，禁用后页面刷新、访问不受限。

锁定界面为紧凑布局：一个密码输入框 + 旁边的小「解锁」按钮，**直接回车即可解锁**。
...
- **锁定**：客户端 Ctrl+L / 页面隐藏触发全屏遮罩，解锁调用 `/auth/unlock` 重新验证密码
  ——即使会话仍在有效期内，主动锁定后也必须重新认证（需求 1 的例外条款）。
...
- **Ctrl+L 无反应**：部分浏览器保留 Ctrl+L（聚焦地址栏）无法拦截；此时请使用
  **切换标签页**（页面隐藏即锁定）或设置面板里的「锁定界面」按钮。
