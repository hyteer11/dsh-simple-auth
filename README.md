...
| 2 | Ctrl+Shift+L 快捷锁定界面 | 客户端快捷键 Ctrl/Cmd+Shift+L（避开浏览器保留的 Ctrl+L）弹出全屏锁定遮罩，解锁需验证密码；左下角侧边栏底部也有「锁定」按钮，一键锁定 |
...
- **锁定**：客户端 Ctrl+Shift+L（或左下角「锁定」按钮）触发全屏遮罩，解锁调用 `/auth/unlock` 重新验证密码
